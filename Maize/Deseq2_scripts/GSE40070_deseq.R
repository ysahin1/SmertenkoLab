...
## load file
###################################################
GSE40070_raw_counts <- read.table("./GSE40070_raw_counts_hisat-sorted.txt", header = TRUE, sep="\t")
head(GSE40070_raw_counts)
row.names(GSE40070_raw_counts) <- GSE40070_raw_counts$Geneid
##########retrieve gene length for GO enrichment
gene_length_GSE40070 <- GSE40070_raw_counts$Length
names(gene_length_GSE40070) <- GSE40070_raw_counts$Geneid
##################################################
GSE40070_raw_counts <- GSE40070_raw_counts[,-c(1:6)] #exclude all information that do not include counts
names(GSE40070_raw_counts) <- c("GSM984656",
                        "GSM984657",
                        "GSM984658",
                        "GSM984659")
GSE40070_grow_CG <- c("control","control","drought","drought")
GSE40070_grow_CG <- factor(GSE40070_grow_CG, levels = c("control","drought"))
GSE40070_grow_CG <- relevel(GSE40070_grow_CG, ref = "control")
####################################
### generate times series deseq object
####################################
GSE40070_sample_info <- data.frame(condition=GSE40070_grow_CG,
                          row.names=names(GSE40070_raw_counts))

dds_GSE40070 <- DESeqDataSetFromMatrix(countData = GSE40070_raw_counts,
                                   colData = GSE40070_sample_info,
                                   design = ~ condition)
...
################################## 
dds_GSE40070 <- dds_GSE40070[rowSums(counts(dds_GSE40070))> 100,]#eleminate counts that are less than zero
length(rowRanges(dds_GSE40070)) # number of genes counted
head(assay(dds_GSE40070))

######normalization
dds_GSE40070 <- estimateSizeFactors(dds_GSE40070)
sizeFactors(dds_GSE40070)
sfn_dds_GSE40070 <- counts(dds_GSE40070, normalized = TRUE)

########transformation
ln_dds_GSE40070 <- log2(sfn_dds_GSE40070+1)


### visulaise norm and log norm data#####
par(mar=c(9,5,3,3))
boxplot(sfn_dds_GSE40070, notch = TRUE,
        main = "Size-Factor Normalized Gene Abundance", 
        ylab = " ",
        las = 2)
axis(1, las=2, outer = FALSE)
par(mar=c(9,5,3,2))
boxplot(ln_dds_GSE40070 , notch = TRUE ,
          main = "Log2-Transformed SizeFactor Normalized Gene Abundance" ,
          ylab = " ",
        las=2)
axis(1, las=2, outer = FALSE)
################# check if your data homoskedastic
plot (ln_dds_GSE40070 [ ,1:2] , cex =.1 , main = " Normalized log2 ( read counts ) " )
################# vst transformation ##########
library(vsn)
vst_dds_GSE40070 <- vst(dds_GSE40070, blind = TRUE)
vstc_dds_GSE40070 <- assay(vst_dds_GSE40070)



######visualize vst norm counts#####
msd_plot <- meanSdPlot (ln_dds_GSE40070,
                           ranks = FALSE, # show the data on the original scale
                           plot = FALSE)
msd_plot$gg +
  ggtitle("Sequencing Depth Normalized log2 Transformed Read Counts") +
  ylab("Standard Deviation")
msd_plot <- meanSdPlot(vst_dds_GSE40070,
                        ranks = FALSE, # show the data on the original scale
                        plot = FALSE)
msd_plot$gg +
...
dis_vstc_dds_GSE40070 <- as.dist(1- cor(vstc_dds_GSE40070, method ="pearson"))

#######visiulaze HC ####
plot(hclust(dis_vstc_dds_GSE40070),
      labels = colnames(dis_vstc_dds_GSE40070),
      xlab = "",
      main = "Hierarchical Clustering of GSE40070")

########### PCA ##########################
pc <- prcomp (t(vstc_dds_GSE40070))

plot(pc$x[ ,1],pc$x[ ,2],
         col = colData(dds_GSE40070)[ ,1],
         main = "Principal Component Analysis")


P <- plotPCA(vst_dds_GSE40070)
############## plot cosmetics ##########################
P <- P + theme_bw() + ggtitle ("Principal Component Analysis of GSE40070")
print (P)

############## DEG analysis ############################
dds_GSE40070 <- DESeq(dds_GSE40070)
dds_GSE40070 <- estimateSizeFactors(dds_GSE40070)
dds_GSE40070 <- estimateDispersions(dds_GSE40070)
dds_GSE40070 <- nbinomWaldTest(dds_GSE40070)

dds_GSE40070_results <- results(dds_GSE40070, 
                                independentFiltering = TRUE, 
                                alpha = 0.05, lfcThreshold = 0)
summary(dds_GSE40070_results)

dn_GSE40070 <- rownames(subset(dds_GSE40070_results, 
