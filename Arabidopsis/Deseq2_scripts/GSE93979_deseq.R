...
## load file
###################################################
GSE93979_raw_counts <- read.table("./GSE93979_raw_counts_hisat-sorted.txt", header = TRUE, sep="\t")
head(GSE93979_raw_counts)
row.names(GSE93979_raw_counts) <- GSE93979_raw_counts$Geneid
##########retrieve gene length for GO enrichment
gene_length_GSE93979 <- GSE93979_raw_counts$Length
names(gene_length_GSE93979) <- GSE93979_raw_counts$Geneid
##################################################
GSE93979_raw_counts <- GSE93979_raw_counts[,-c(1:6)] #exclude all information that do not include counts
names(GSE93979_raw_counts) <- c("GSM2465998",
                        "GSM2465999",
                        "GSM2465601",
                        "GSM2465602")
GSE93979_grow_CG <- c("control","control","drought","drought")

####################################
### generate times series deseq object
####################################
GSE93979_sample_info <- data.frame(condition=GSE93979_grow_CG,
                          row.names=names(GSE93979_raw_counts))

dds_GSE93979 <- DESeqDataSetFromMatrix(countData = GSE93979_raw_counts,
                                   colData = GSE93979_sample_info,
                                   design = ~ condition)
...
################################## 
dds_GSE93979 <- dds_GSE93979[rowSums(counts(dds_GSE93979))> 100,]#eleminate counts that are less than zero
length(rowRanges(dds_GSE93979)) # number of genes counted
head(assay(dds_GSE93979))

######normalization
dds_GSE93979 <- estimateSizeFactors(dds_GSE93979)
sizeFactors(dds_GSE93979)
sfn_dds_GSE93979 <- counts(dds_GSE93979, normalized = TRUE)

########transformation
ln_dds_GSE93979 <- log2(sfn_dds_GSE93979+1)


### visulaise norm and log norm data#####
par(mar=c(9,5,3,3))
boxplot(sfn_dds_GSE93979, notch = TRUE,
        main = "Size-Factor Normalized Gene Abundance", 
        ylab = " ",
        las = 2)
axis(1, las=2, outer = FALSE)
par(mar=c(9,5,3,2))
boxplot(ln_dds_GSE93979 , notch = TRUE ,
          main = "Log2-Transformed SizeFactor Normalized Gene Abundance" ,
          ylab = " ",
        las=2)
axis(1, las=2, outer = FALSE)
################# check if your data homoskedastic
plot (ln_dds_GSE93979 [ ,1:2] , cex =.1 , main = " Normalized log2 ( read counts ) " )
################# vst transformation ##########
library(vsn)
vst_dds_GSE93979 <- vst(dds_GSE93979, blind = TRUE)
vstc_dds_GSE93979 <- assay(vst_dds_GSE93979)



######visualize vst norm counts#####
msd_plot <- meanSdPlot (ln_dds_GSE93979,
...
dis_vstc_dds_GSE93979 <- as.dist(1- cor(vstc_dds_GSE93979, method ="pearson"))

#######visiulaze HC ####
plot(hclust(dis_vstc_dds_GSE93979),
      labels = colnames(dis_vstc_dds_GSE93979),
      xlab = "",
      main = "Hierarchical Clustering of GSE93979")

########### PCA ##########################
pc <- prcomp (t(vstc_dds_GSE93979))

plot(pc$x[ ,1],pc$x[ ,2],
         col = colData(dds_GSE93979)[ ,1],
         main = "Principal Component Analysis")


P <- plotPCA(vst_dds_GSE93979)
############## plot cosmetics ##########################
P <- P + theme_bw() + ggtitle ("Principal Component Analysis of GSE93979")
print (P)

############## DEG analysis ############################
dds_GSE93979 <- DESeq(dds_GSE93979)
dds_GSE93979 <- estimateSizeFactors(dds_GSE93979)
dds_GSE93979 <- estimateDispersions(dds_GSE93979)
dds_GSE93979 <- nbinomWaldTest(dds_GSE93979)

dds_GSE93979_results <- results(dds_GSE93979, 
                                independentFiltering = TRUE, 
                                alpha = 0.05, lfcThreshold = 0)
summary(dds_GSE93979_results)

dn_GSE93979 <- rownames(subset(dds_GSE93979_results, 
