...
BiocManager::install(c("heatmap.2"))
...
## load file
###################################################
GSE134945_raw_counts <- read.table("./GSE134945_raw_counts_hisat-sorted.txt", header = TRUE, sep="\t")
head(GSE134945_raw_counts)
row.names(GSE134945_raw_counts) <- GSE134945_raw_counts$Geneid
##########retrieve gene length for GO enrichment
gene_length_GSE134945 <- GSE134945_raw_counts$Length
names(gene_length_GSE134945) <- GSE134945_raw_counts$Geneid
##################################################
GSE134945_raw_counts <- GSE134945_raw_counts[,-c(1:6)] #exclude all information that do not include counts
names(GSE134945_raw_counts) <- c("GSM3980153",
                        "GSM3980154",
                        "GSM3980155",
                        "GSM3980159",
                        "GSM3980160",
                        "GSM3980161")
GSE134945_grow_CG <- c("control","control","control",
                            "drought","drought","drought")

####################################
### generate times series deseq object
####################################
GSE134945_sample_info <- data.frame(condition=GSE134945_grow_CG,
                          row.names=names(GSE134945_raw_counts))

dds_GSE134945 <- DESeqDataSetFromMatrix(countData = GSE134945_raw_counts,
                                   colData = GSE134945_sample_info,
                                   design = ~ condition)
...
################################## 
dds_GSE134945 <- dds_GSE134945[rowSums(counts(dds_GSE134945))> 100,]#eleminate counts that are less than zero
length(rowRanges(dds_GSE134945)) # number of genes counted


######normalization
dds_GSE134945 <- estimateSizeFactors(dds_GSE134945)
sizeFactors(dds_GSE134945)
sfn_dds_GSE134945 <- counts(dds_GSE134945, normalized = TRUE)

########transformation
ln_dds_GSE134945 <- log2(sfn_dds_GSE134945+1)


### visulaise norm and log norm data#####
par(mar=c(9,5,3,3))
boxplot(sfn_dds_GSE134945, notch = TRUE,
        main = "Size-Factor Normalized Gene Abundance of GSE134945", 
        ylab = " ",
        las = 2)
axis(1, las=2, outer = FALSE)
par(mar=c(9,5,3,2))
boxplot(ln_dds_GSE134945 , notch = TRUE ,
          main = "Log2-Transformed SizeFactor Normalized Gene Abundance of GSE134945" ,
          ylab = " ",
        las=2)
axis(1, las=2, outer = FALSE)
################# check if your data homoskedastic
plot (ln_dds_GSE134945 [ ,1:2] , cex =.1 , main = " Normalized log2 ( read counts ) " )
################# vst transformation ##########
library(vsn)
vst_dds_GSE134945 <- vst(dds_GSE134945, blind = TRUE)
vstc_dds_GSE134945 <- assay(vst_dds_GSE134945)



######visualize vst norm counts#####
msd_plot <- meanSdPlot (ln_dds_GSE134945,
                           ranks = FALSE, # show the data on the original scale
                           plot = FALSE)
msd_plot$gg +
  ggtitle("Sequencing Depth Normalized log2 Transformed Read Counts") +
  ylab("Standard Deviation")
msd_plot <- meanSdPlot(vst_dds_GSE134945,
                        ranks = FALSE, # show the data on the original scale
                        plot = FALSE)
msd_plot$gg +
...
dis_vstc_dds_GSE134945 <- as.dist(1- cor(vstc_dds_GSE134945, method ="pearson"))

#######visiulaze HC ####
plot(hclust(dis_vstc_dds_GSE134945),
      labels = colnames(dis_vstc_dds_GSE134945),
      xlab = "",
      main = "Hierarchical Clustering of GSE134945")

########### PCA ##########################
pc <- prcomp (t(vstc_dds_GSE134945))

plot(pc$x[ ,1],pc$x[ ,2],
         col = colData(dds_GSE134945)[ ,1],
         main = "Principal Component Analysis of GSE134945")


P <- plotPCA(vst_dds_GSE134945)
############## plot cosmetics ##########################
P <- P + theme_bw() + ggtitle ("Principal Component Analysis")
print (P)

############## DEG analysis ############################
dds_GSE134945 <- DESeq(dds_GSE134945)
dds_GSE134945 <- estimateSizeFactors(dds_GSE134945)
dds_GSE134945 <- estimateDispersions(dds_GSE134945)
dds_GSE134945 <- nbinomWaldTest(dds_GSE134945)

dds_GSE134945_results <- results(dds_GSE134945, 
                                independentFiltering = TRUE, 
                                alpha = 0.05, lfcThreshold = 0)
summary(dds_GSE134945_results)

dn_GSE134945 <- rownames(subset(dds_GSE134945_results, 
